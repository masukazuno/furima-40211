  ...
  describe '配送先情報の保存' do
    context '配送先情報の保存ができるとき' do
      ...
      it '都道府県が「---」以外かつ空でなければ保存できる' do
        @order_form.prefecture_code_id = 2
        expect(@order_form).to be_valid
      end
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '都道府県が「---」だと保存できないこと' do
        @order_form.prefecture_code_id = 1
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Prefecture code can't be blank")
      end
      it '都道府県が空だと保存できないこと' do
        @order_form.prefecture_code_id = nil
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Prefecture code can't be blank", "Prefecture code can't be blank")
      end
